^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package im_publisher
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
3.0.0 (2022-04-29)
------------------
* Cleanup im_publisher.
* im_publisher: Fix out_img timestamp for using with sim time (`#735 <https://github.com/ros-perception/image_pipeline/issues/735>`_)
...
* [Foxy] Use ament_auto Macros (`#573 <https://github.com/ros-perception/image_pipeline/issues/573>`_)
  ...
  * Applying ament_auto macros to im_publisher.
...
2.0.0 (2018-12-09)
------------------
* port im_publisher on ROS2 (`#366 <https://github.com/ros-perception/image_pipeline/issues/366>`_)
* Initial ROS2 commit.
...
* fix 'VideoCapture' undefined symbol error (`#318 <https://github.com/ros-perception/image_pipeline/issues/318>`_)
  * fix 'VideoCapture' undefined symbol error
  The following error occured when trying to run im_publisher:
  [...]/devel/lib/im_publisher/im_publisher: symbol lookup error: [...]/devel/lib//libim_publisher.so: undefined symbol: _ZN2cv12VideoCaptureC1Ev
  ...
  * add im_publisher opencv 2 compatibility
...
* explicitly cast to std::vector<double> to make gcc6 happy
  With gcc6, compiling im_publisher fails with this error:
  ```
  /[...]/im_publisher/src/nodelet/im_publisher_nodelet.cpp: In member function 'virtual void im_publisher::ImagePublisherNodelet::onInit()':
  /[...]/im_publisher/src/nodelet/im_publisher_nodelet.cpp:180:43: error: ambiguous overload for 'operator=' (operand types are 'sensor_msgs::CameraInfo\_<std::allocator<void> >::_D_type {aka std::vector<double>}' and 'boost::assign_detail::generic_list<int>')
  ...
  ```
  | /[...]/im_publisher/src/nodelet/im_publisher_nodelet.cpp: In member function 'virtual void im_publisher::ImagePublisherNodelet::onInit()':
  | /[...]/im_publisher/src/nodelet/im_publisher_nodelet.cpp:180:65: error: call of overloaded 'vector(boost::assign_detail::generic_list<int>&)' is ambiguous
  ...
  suggestions and with that im_publisher compiles with gcc6.
